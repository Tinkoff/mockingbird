...
declare type If<B, F, S> = B extends true ? F : S;
declare type Or<B1, B2> = B1 extends true ? true : B2;
declare type Not<B> = B extends true ? false : true;
declare type Has<U extends any, U1 extends any> = [U1] extends [U] ? true : false;
declare type IsExactKey<T> = string extends T ? false : number extends T ? false : symbol extends T ? false : true;
declare type ValueByPath<P, O, U = false> = P extends readonly [infer F, ...(infer R)] ? Or<Has<O, null>, Has<O, undefined>> extends infer HasNullOrUndefined ? Exclude<O, undefined | null> extends infer O2 ? O2 extends object ? F extends keyof O2 ? R extends [] ? If<Or<U, HasNullOrUndefined>, O2[F] | undefined, O2[F]> : ValueByPath<R, O2[F], Or<Or<U, HasNullOrUndefined>, 
/**
 ...
 */
Not<IsExactKey<keyof O2>>>> : undefined : never : never : never : never;
